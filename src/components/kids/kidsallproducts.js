...
const Kidsallproducts = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState([]);
    ...
    useEffect(() => {
        setProducts(productList.filter(item => item.producttype === "kids"));
        setFilteredProducts(productList.filter(item => item.producttype === "kids"))
    },[]);
    ...
    return (
        <>
             <MyNavbar/>
            <h3 className='ps-lg-5 ps-2 text-center' style={{textShadow:"2px 3px 2px gray"}}>Kids Zone</h3>
