...
export default function Sellerproducts() {
  const {values, adminaccepted, setAdminaccepted}=useCart();
              ...
              <div className="border m-3 rounded">
                <div className="table-responsive p-3">
                  ...
                  >
                    ...
                    <tbody>
                      {adminaccepted && data.map((item,index)=>(
                        <tr key={index}>
                          <td>{index+1}</td>
                          {/* <td><img src={item.productimageurl} alt="product image" width="150" height="150"/></td> */}
                          <td><img src={item.productimageurl} alt="product"  width="150" height="150"/></td>
                          <td>{item.productname}</td>
                          <td>{item.price}</td>
                          <td>Available</td>
                          <td>{}</td>
                          <td><button className="btn btn-danger" onClick={()=>{setAdminaccepted(false)}}>Delete</button></td>
                          {/* <td>
                            <button className="btn btn-success m-1" onClick={()=>{setAdminaccepted(true)}}>Approve</button>
                            <button className="btn btn-danger m-1" onClick={()=>{setAdminaccepted(false)}}>Reject</button>
                          </td> */}
                        </tr>
                      ))}
                    </tbody>
